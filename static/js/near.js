(function () {
  ...
  // document.getElementById("nota-about").innerText = `День появления через`;
...
function toDates(data) {
  return data.map((x) => {
    ...
    return {
      name: x.name,
      birth: new Date(now.getFullYear(), parseInt(bd[1]) - 1, parseInt(bd[0])),
...
function followedby(data) {
  return data
    .map((x) => `${x.name} | ${x.birth.toLocaleDateString()} | явление миру\n`)
    .join("\n");
}
